...
class _DataVisState extends State<DataVis> with FeatureShower {
  ...
  Future<void> _getData() async {
    var data = await sendToServer('display_data');
    ...
    if (data is Map) {
      ...
      for (var key in data.keys) {
        ...
        var mcap = item;
        if (mcap is String) {
          mcap = double.parse(mcap);
        }

        var pd = (_PieData(symbol, mcap));
...
Machine learning through Keras or APIs can make your app more intelligent.""";
  @override
  String bgImage() => 'images/data anal.jpg';
  ...
  String numberSummarizer(var value) {
    // return value.toString();
    String sigFig(double v) {
      return v.toStringAsFixed(1);
    }

    if (value > 1e12) {
      value /= 1e12;
      return '${sigFig(value)} T \$';
    }
    if (value > 1e9) {
      value /= 1e9;
      return '${sigFig(value)} B \$';
    }
    if (value > 1e6) {
      value /= 1e6;
      return '${sigFig(value)} m \$';
    }
    if (value > 1e3) {
      value /= 1e3;
      return '${sigFig(value)} k \$';
    }
    return '${sigFig(value)} \$';
  }
    ...
    return visWrap(SfCircularChart(
        ...
        series: <PieSeries<_PieData, String>>[
          PieSeries<_PieData, String>(
              ...
              //     data.yData.toInt().toString().padLeft(15, '0'),
              xValueMapper: (_PieData data, _) => data.xData,
              yValueMapper: (_PieData data, _) => data.yData,
              dataLabelMapper: (_PieData data, _) =>
                  numberSummarizer(data.yData),
