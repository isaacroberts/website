...
class _HeaderState extends State<Header> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return SliverToBoxAdapter(child: fullViewRebuild(context));
    return SliverToBoxAdapter(
        child: LayoutBuilder(builder: (context, constraints) {}));
    return SizedBox(
        width: Device.width,
        height: Device.height - appBar,
        child: fullViewRebuild(context));
  ...
  Widget fullViewRebuild(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      double h = Device.height - appBar;

      double w = constraints.maxWidth;
      const double minW = 53 * k + standardContainerMargin * 2;
      const double maxW = processWidth + standardContainerMargin * 2;
      double textW = w - h * .85;
      log('textW = $textW');
      if (w > maxW) {
        return Center(
            child: SizedBox(
                width: maxW,
                height: h,
                child: fullViewRebuildContent(context)));
      } else if (w < 500) {
        return SizedBox(height: h, width: w, child: tinyViewRebuild(context));
        // } else {
        //   return SizedBox(height: h, width: w, child: medViewRebuild(context));
        // }
        // if (false) {
      } else if (textW < 318) {
        return SizedBox(height: h, width: w, child: smallViewRebuild(context));
        // return SizedBox(height: h, child: OverflowBox(
        //   alignment: Alignment.centerLeft,
        //     minWidth: minW, minHeight: h, maxHeight: h, maxWidth: 1200,
        //     child:
        // SizedBox(width: minW, height: h, child:
        // fullViewRebuildContent(context))));
      } else {
        return SizedBox(
            width: w,
            height: h,
            child: Center(child: fullViewRebuildContent(context)));
      }
    });
  }

  Widget fullViewRebuildContent(BuildContext context) {
    return Padding(
        padding:
            const EdgeInsets.symmetric(horizontal: standardContainerMargin),
        child: Stack(
            key: const Key('fullViewStackHeader'),
            fit: StackFit.expand,
            children: [
              Positioned.fill(
                  child: Align(
                  ...
                  children: [
                    Expanded(
                        child: Center(
                            child: Padding(
                                padding:
                                    const EdgeInsets.all(textContainerMargin),
                                child: SingleChildScrollView(
                                    child:
                                        introText(context, leftAlign: true))))),
  ...
  Widget medViewRebuild(BuildContext context) {
    return Padding(
        padding:
            const EdgeInsets.symmetric(horizontal: standardContainerMargin),
        child: Stack(
                        ...
                        child: Center(
                            child: Padding(
                                ...
                                child: SingleChildScrollView(
                                    child:
                                        introText(context, leftAlign: true))))),
                    const Expanded(child: ProPic()),
                  ])
            ]));
  }

  Widget smallViewRebuild(BuildContext context) {
    return Stack(
      children: [
  ...
  Widget tinyViewRebuild(BuildContext context) {
    return Stack(
      children: [
        const ProPic(isThin: true, roundedCorner: false),
        Center(
      ...
      clipper(
        ...
        Flow(
            ...
            children: [
              ...
              fadeAssetBg('images/palm beige.jpg',
