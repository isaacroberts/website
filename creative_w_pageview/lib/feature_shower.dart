...
abstract class FeatureShower {
  ...
  Widget parallaxImg(BuildContext context) {
    return ParallaxImage(
        image: bgImage(), opacity: .5, distance: 1.5, debugLabel: title());
  }
  ...
  Widget vertTextContainer(BuildContext context) {
    return Padding(
        padding: const EdgeInsets.fromLTRB(
            standardContainerMargin,
            standardContainerMargin + 15 + appBar,
            standardContainerMargin,
            standardContainerMargin + 15),
        child: Center(
            child: SizedBox(
                height: 600,
                child: parallax(context,
                    child: InkWell(
  ...
  List<Widget> children(BuildContext context, bool left, bool vertical) {
    if (vertical) {
      ...
      if (left) {
        return [
          Flexible(child: visWrap(textContainer(context, left))),
  ...
  Widget visWrap(Widget child) {
    return VisibilityDetector(
        key: const Key('feature_vis'),
        onVisibilityChanged: (visibilityInfo) {
          if (!hasBeenVis) {
            if (visibilityInfo.visibleFraction >= .98) {
              hasBeenVis = true;
